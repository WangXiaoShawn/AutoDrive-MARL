...
def greedy_evaluation(agent_filename, env_name, num_tests, output_image, device):
    try:
        ...
        if 'policy_state_dict' not in checkpoint:
            raise KeyError("The checkpoint does not contain 'policy_state_dict'.")
        
        policy_state_dict = checkpoint['policy_state_dict']
        input_dim = policy_state_dict['fc.0.weight'].shape[1]
        action_dim = policy_state_dict['fc.4.weight'].shape[0]
...
def main():
    agent_filename = 'xwang277_jiaxingl_final_TrafficJunction10_a3c_share.pkl' 
    env_name = 'ma_gym:TrafficJunction10-v1'  
    num_tests = 10 
    output_image = 'xwang277_jiaxingl_final_TrafficJunction10_a3c_share_greedy_test_plot'
