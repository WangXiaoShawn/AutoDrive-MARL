...
def greedy_evaluation(agent_filename, env_name, num_tests, output_image, device):
    try:
        ...
        if 'policy_state_dicts' not in checkpoint:
            raise KeyError("The checkpoint does not contain 'policy_state_dicts'.")
        policy_state_dicts = checkpoint['policy_state_dicts']
        num_agents = len(policy_state_dicts)
        print(f"Number of agents detected: {num_agents}")
        policy_nets = []
        for i, state_dict in enumerate(policy_state_dicts):
...
def main():
    agent_filename = 'xwang277_jiaxingl_final_TrafficJunction10_a3c_individual.pkl'  
    env_name = 'ma_gym:TrafficJunction10-v1' 
    num_tests = 10 
    output_image = 'xwang277_jiaxingl_final_TrafficJunction10_a3c_individual_greedy_test_plot.png'  
