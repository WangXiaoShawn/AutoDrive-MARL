...
def greedy_evaluation(agent_filename, env_name, num_tests, output_image, device):
    try:
        ...
        if 'actor_state_dict' not in checkpoint:
            raise KeyError("The checkpoint does not contain 'actor_state_dict'.")
        
        actor_state_dict = checkpoint['actor_state_dict']
        input_dim = actor_state_dict['fc.0.weight'].shape[1]
        action_dim = actor_state_dict['fc.4.weight'].shape[0]
        policy_net = Actor(input_dim=input_dim, action_dim=action_dim).to(device)
        policy_net.load_state_dict(actor_state_dict)
        policy_net.eval()
        print(f"Loaded policy network with input_dim={input_dim} and action_dim={action_dim}")
...
def main():
    ...
    agent_filename = 'xwang277_jiaxingl_final_TrafficJunction10_a3c_ppo_share.pkl' 
    num_tests = 10  
    output_image = 'xwang277_jiaxingl_final_TrafficJunction10_a3c_ppo_share_greedy_plot.png'  
