...
class SharedA3CAgent:
    def __init__(self, action_dim, input_dim, env_name, algorithm_name, num_agents, num_workers=8, T_max=5, T_max_global=int(1e7), log_file="a3c_log.json"):
        ...
        self.global_policy_net = Actor(input_dim=input_dim, action_dim=action_dim).to(self.device)
        self.global_value_net = Critic(input_dim=input_dim).to(self.device)
        self.global_policy_net.share_memory()
        self.global_value_net.share_memory()
        self.optimizer = SharedAdam(list(self.global_policy_net.parameters()) + list(self.global_value_net.parameters()), lr=self.learning_rate)
    ...
    def evaluate(self, env, num_episodes=10):
        ...
        try:
                    ...
                    while not all(done) and step < max_steps:
                        ...
                        for i in range(self.num_agents):
                            if not done[i]:
                                ...
                                with torch.no_grad():
                                    logits = self.global_policy_net(state_tensor)
            ...
            if avg_reward_over_episodes > self.best_performance:
                ...
                checkpoint_filename = f'best_shared_a3c_checkpoint_{self.env_name.replace("/", "_")}_reward{avg_reward_over_episodes:.2f}.pkl'
            ...
            self.global_policy_net.train()
            self.model_lock.release()
            print("Evaluation finished, resuming training.")
    def save_checkpoint(self, filename="checkpoint.pkl"):
        checkpoint = {
            'policy_state_dict': self.global_policy_net.state_dict(),
            'value_state_dict': self.global_value_net.state_dict(),
    ...
    def load_checkpoint(self, filename="checkpoint.pkl"):
        ...
        self.global_policy_net.load_state_dict(checkpoint['policy_state_dict'])
        self.global_value_net.load_state_dict(checkpoint['value_state_dict'])
    ...
    def worker(self, worker_id):
        ...
        local_policy_net = Actor(input_dim=self.input_dim, action_dim=self.action_dim).to(self.device)
        local_value_net = Critic(input_dim=self.input_dim).to(self.device)
        local_policy_net.train()
        local_value_net.train()
        ...
        while True:
            ...
            total_reward = [0] * self.num_agents
            local_policy_net.load_state_dict(self.global_policy_net.state_dict())
            local_value_net.load_state_dict(self.global_value_net.state_dict())
            ...
            while not all(done):
                ...
                for _ in range(self.T_max):
                    ...
                    for i in range(self.num_agents):
                        if not done[i]:
                            ...
                            logits = local_policy_net(state_tensor)
                            value = local_value_net(state_tensor)
                            probs = F.softmax(logits, dim=-1)
                ...
                for i in range(self.num_agents):
                    ...
                    if not done[i]:
                        state_tensor = torch.FloatTensor(state[i]).unsqueeze(0).to(self.device)
                        R = local_value_net(state_tensor).detach()
                    values[i].append(R)
                    policy_loss = 0
                    value_loss = 0
                    ...
                    total_loss.backward()
                    nn.utils.clip_grad_norm_(local_policy_net.parameters(), self.max_grad_norm)
                    nn.utils.clip_grad_norm_(local_value_net.parameters(), self.max_grad_norm)
                    ensure_shared_grads(local_policy_net, self.global_policy_net)
                    ensure_shared_grads(local_value_net, self.global_value_net)
    ...
    def train_a3c(self, evaluation_interval=100000, num_evaluations=10):
        ...
        final_checkpoint = f"best_shared_a3c_checkpoint_{self.env_name.replace('/', '_')}_{self.algorithm_name}.pkl"
...
def main():
    ...
    agent = SharedA3CAgent(
        action_dim=action_dim,
        input_dim=input_dim,
        env_name=env_name,
        algorithm_name='SharedA3C',
        num_agents=num_agents,
        num_workers=8,
        T_max=20,
        T_max_global=int(1e7),
        log_file="shared_a3c_multiagent_log.json"
    )
